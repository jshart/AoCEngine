...
class Matrix:
    ...
    def randomiseMatrix(self,min,max):
        for y in range(0, self.height):
            for x in range(0, self.width):
                self.setCell(x, y, random.randint(min, max))
    ...
    def subMatrix(self, x1, y1, x2, y2):
        ...
        for y in range(y1, y2+1):
            for x in range(x1, x2+1):
                newMatrix.setCell(x-x1, y-y1, self.getCell(x, y).value)
    ...
    def processCommands(self, sc, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_0:
                self.switchMatrix(sc, sc.matrices[0])
            elif event.key == pygame.K_1:
                self.switchMatrix(sc, sc.matrices[1])
...
for y in range(1, matrix.height):
    for x in range(1, matrix.width):
        matrix.setCell(x, y, doCalcs(x, y, sNumber))
