...
cpu.attachInputBuffer(robotToCPU)
cpu.attachOutputBuffer(CPUToRobot)
...
sm = SpriteManager()
paintRobot=Robot(initMatrixX//2, initMatrixY//2, cellSizeW, cellSizeH)
sm.addSprite(paintRobot.sprite)
# lets link the IO buffers
paintRobot.attachInputBuffer(CPUToRobot)
paintRobot.attachOutputBuffer(robotToCPU)
...
paintRobot.sprite.setCellAtCurrentLocation(vdu, 1)
paintColour=paintRobot.sprite.getCellValueAtCurrentLocation(vdu)
#print("Initial paintColour:", paintColour)
# add this into the buffer to the CPU
paintRobot.output.write(paintColour)
...
#while running:
    ...
    if len(paintRobot.input.buffer)>=2:
        paintRobot.recordLocation()
        paintColour=paintRobot.input.read()
        robotNewDirection=paintRobot.input.read()

        #print("paintColour:", paintColour, "robotNewDirection:", robotNewDirection)

        paintRobot.sprite.setCellAtCurrentLocation(vdu,paintColour)
        if robotNewDirection==1:
            paintRobot.sprite.turnRight()
        else:
            paintRobot.sprite.turnLeft()

        paintRobot.sprite.moveForward()

        # get the paint colour under the robot
        paintColour=paintRobot.sprite.getCellValueAtCurrentLocation(vdu)
        #print("New paintColour:", paintColour)
        # add this into the buffer to the CPU
        paintRobot.output.write(paintColour)
    else:
...
print("Robot location history")
print(paintRobot.locationHistory)
print("count:")
print(len(paintRobot.locationHistory))
