...
totalSafe=0
while runs<maxRuns:
    safe=line1.count('.')-2 # need to ignore/remove the bookend tiles we add
    totalSafe+=safe
    print("run:",runs,"=[",line1[1:-1],"] safe=", safe, "totalSafe=", totalSafe)

    # look at each character in turn
    for i in range(len(line1)-sampleRange+1):
        # next sampleRange characters as a substring:
        sample=""

        # calculate the "current 3 chars" as a sample
        for s in range(i, i+sampleRange):
            sample+=line1[s]
        #print("[",i,"]->",sample,"<-[",s,"]=",(sample in trapPatterns))
        
        # check to see if this sampke is one of the trap patterns
...
print("Final safe count:",totalSafe)
